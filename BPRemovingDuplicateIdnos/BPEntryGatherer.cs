...
public class BPEntryGatherer
{
    ...
    public BPEntryGatherer( Logger? logger)
    {
        logger?.LogProcessingInfo($"Created BPEntryGatherer");
        this.logger = logger;
    }

    Logger? logger { get; }

    public BPDataEntry? GetEntry(string bpNumber)
    {
        logger?.Log($"Gathering BPEntry {bpNumber}");
        logger?.LogProcessingInfo($"Gathering BPEntry {bpNumber}");
        BPDataEntry? entry = null;
        try
        {
            var URL = $"https://bibpap.be/BP_enl/?fs=2&n={bpNumber}";
            ...
            if (table != null)
            {
                ...
                entry = new BPDataEntry(bpNumber, logger);
        ...
        {
            logger?.LogError($"Error in gathering BP entry {bpNumber}", e);
            Console.WriteLine(e);
        }
    ...
    private BPDataEntry? GetEntry(int currentYear, int currentIndex)
    {
        try
        {
            ...
            var bpText = $"{yearText}-{indexText}";
            return GetEntry(bpText);
        }
        catch (Exception e)
        {
